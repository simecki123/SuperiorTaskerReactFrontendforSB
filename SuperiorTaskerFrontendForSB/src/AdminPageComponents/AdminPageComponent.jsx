...
import { deleteUser, fetchAllUsers, updateUserRole } from '../services/api'; // Assuming updateUserRole is defined in the API services
...
function AdminPageComponent() {
    ...
    useEffect(() => {
        ...
        const fetchUsers = async () => {
            setLoading(true);
            try {
                const users = await fetchAllUsers(storedUser.id);
                ...
                <div className="user-list-container">
                    <ul className='project-part-unordered-list'>
                        {filteredUserList.map(user => (
                            <li className='project-part-list' key={user.id}>
                                <UserCardComponent 
                                    user={user} 
